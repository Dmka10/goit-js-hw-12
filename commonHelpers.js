...
</li>`).join("");m.imageList.innerHTML=r}document.querySelector(".search-form"),document.querySelector(".search-input"),document.querySelector(".search-btn"),document.querySelector(".images-list");function p(i){const r="https://pixabay.com",o="/api/",a=new URLSearchParams({key:"44431585-8d368e0ee08bb3db1ccb0f280",q:i,image_type:"photo",orientation:"horizontal",safesearch:!0,page:1,per_page:30}),e=`${r}${o}?${a}`;return console.log(e),fetch(e).then(t=>{if(t.ok)return t.json();throw new Error(t.status)})}const s={imageSearchForm:document.querySelector(".search-form"),imageSearchInput:document.querySelector(".search-input"),submitButton:document.querySelector(".search-btn"),imageList:document.querySelector(".images-list"),loader:document.querySelector(".loader")};s.loader.style.display="none";let c;s.imageSearchForm.addEventListener("submit",i=>{if(i.preventDefault(),c=i.target.elements.userData.value,c.trim()==="")return s.imageList.innerHTML="",l.info({message:"You need to enter search request!",position:"topRight",transitionIn:"bounceInDown",transitionOut:"fadeOutDown"});document.querySelector(".loader"),s.loader.style.display="block",p(c).then(r=>{if(r.hits.length===0)return s.imageList.innerHTML="",l.error({message:"Sorry, there are no images matching your search query. Please try again!",position:"topRight",transitionIn:"bounceInDown",transitionOut:"fadeOutDown"});d(r),new u(".images-list-item a",{captions:!0,captionSelector:"img",captionType:"attr",captionsData:"alt",captionPosition:"bottom",captionDelay:250,animationSpeed:300,widthRatio:1,heightRatio:.95,disableRightClick:!0}).refresh()}).then(()=>s.loader.style.display="none").catch(r=>{console.log(r),s.loader.style.display="none"}),i.target.reset()});
